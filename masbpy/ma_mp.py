...
from torch.multiprocessing import Process, cpu_count, Manager
...
class MASB(object):
    ...
    def compute_balls(self, num_processes=cpu_count()):
        ...
        n = int(num_processes / 2)  # we are spawning 2 processes (inner and outer ma) per n
        batchsize = datalen // n
        chunk_bounds = []
        end = 0
        for i in range(n - 1):
    ...
    # @autojit
    # @profile
    def __call__(self, queue, start, end, inner=True, verbose=True):
        verbose = True
        ...
        ma_coords[:] = -1  # nan # changed from nan to -1, as negative coordinates can't exist in our system
        # ma_radii = zeros(m)
        # ma_radii[:] = nan
        ma_f2 = zeros(m, dtype=np.uint32)
        ma_f2[:] = -1  # nan # changed from nan to -1, as negative coordinates can't exist in our system
        ...
        for i, pi in enumerate(range(start, end)):
            ...
            if r_ == None:
                pass
            elif r_ > self.SuperR or r_ == self.SuperR:
                pass
            else:
                # ma_radii[i] = r_
                ma_coords[i] = c
                ma_f2[i] = q_i
                print("ma_coords[" + str(i)+"] = " + str(ma_coords[i]))
        ...
        print("done!", " start: ", start, "inner: ", inner, " ma_coords.shape: ", ma_coords.shape)
